...
# New inspector and phantom
import inspector, phantom
# To get argv
from sys import argv
# To know if the validation_tests file is empty
from os import stat
...
class Validation_Test:
    ...
    def __init__(self, mode):
        ...
        self._mode = mode
        self._player0 = self._player1 = None
        self._threads = []
        if mode == 0:
            #  dummy0 vs dummy1
            self._player0 = 'dummy0'
            self._player1 = 'dummy1'
        elif mode == 1:
            # dummy0 vs phantom
            self._player0 = 'dummy0'
    ...
    def __init_match(self):
        ...
        if self._mode == 0:
            #  dummy0 vs dummy1
            self._threads.append(Thread(target=(dummy0.lancer)))
            self._threads.append(Thread(target=(dummy1.lancer)))
        elif self._mode == 1:
            # dummy0 vs phantom
            self._threads.append(Thread(target=(dummy0.lancer)))
            ...
            # inspector vs dummy1
            self._threads.append(Thread(target=(inspector.lancer)))
            self._threads.append(Thread(target=(dummy1.lancer)))
        elif self._mode == 3:
            # inspector vs phantom
            self._threads.append(Thread(target=(inspector.lancer)))
    ...
    def launch(self, nb):
        ...
        if self._player0 is None:
            print('Bad argument: please read README.md')
            return None
        scores = 0
        winners = [0, 0]
        ...
        for i in range(nb):
            ...
            winner, score = self.__get_winner_score()
            if winner == 0:
                winners[0] += 1
            else:
                winners[1] += 1
...
if len(argv) > 1:
    # Launch a validation test
    joueurs = None
    val = Validation_Test(int(argv[1]))
    if len(argv) > 2 and argv[2].isdigit():
        val.launch(int(argv[2]))
    else:
        val.launch(10)
else:
